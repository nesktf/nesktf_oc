...
function state:init()
  -- self.handle.lsc = self.initLSC(self)
  self.setSides(self)
  -- self.update(self)
end
...
function state:initLSC()
  local gt_machines = component.list("gt_machine", true)

  for address, _ in pairs(gt_machines) do
...
function state:updateLSC()
  local function parse_fuzzy_int(str)
    local filtered_str = string.gsub(str, "([^0-9]+)", "")
    return math.floor(tonumber(filtered_str))
  end

  local function tryUpdate(state_obj)
    local sensor = state_obj.handle.lsc.getSensorInformation()
    state_obj.lsc[LSC_STORED]  = parse_fuzzy_int(sensor[2])
    state_obj.lsc[LSC_MAX]     = parse_fuzzy_int(sensor[3])
    state_obj.lsc[LSC_PLOSS]   = parse_fuzzy_int(sensor[4])
    state_obj.lsc[LSC_AVGIN]   = parse_fuzzy_int(string.gsub(sensor[7], "(last 5 seconds)", ""))
    state_obj.lsc[LSC_AVGOUT]  = parse_fuzzy_int(string.gsub(sensor[8], "(last 5 seconds)", ""))
    state_obj.lsc[LSC_MAINT]   = (string.find(sensor[9], "Has Problems") ~= nil)
    state_obj.lsc[LSC_RATIO]   = state_obj.lsc[LSC_STORED]/state_obj.lsc[LSC_MAX]
  end


  if (not self.lsc[LSC_AVAIL]) then
    self.handle.lsc = self.initLSC(self)
    if (self.handle.lsc == nil) then
      return
    end
  end
  self.lsc[LSC_AVAIL] = true

  local status, err = pcall(tryUpdate, self)
  if not status then
    self.lsc[LSC_AVAIL] = false
  end
...
local widgets = {
  ...
  },
  {
    ...
    draw = function (this)
      ...
      gpu.set(x, y+2, "Can start:       " .. (reactor.canStart(reactor) and "true" or "false"))
      gpu.set(x, y+3, "Is Operating:    " .. (reactor.isOperating and "true" or "false"))
      if (reactor.isOperating) then
        gpu.set(x, y+4, "Can continue:    "..(reactor.canOperate(reactor) and "true" or "false"))
      end
    end
  },
  {
...
local function main()
  ...
  local function guardedMain()
    ...
    local on_touch_event_id = event.listen("touch", function(e, add, x, y, but, play)
